...
public class rtNoiseReducer {
    ...
    public double[] audioSE(double[] doubleData) {
        ...
        in_buffer = audioToBuffer(in_buffer, doubleData); // DoubleData = 512 in_buffer = 512 (3840)
        ...
        realTranformedBuffer = getComplexSlice(transformedBuffer, 0, in_buffer.length / 2 + 1);
...
//        float[][][] out_mask = reshapeAndPadding(remags);
        ...
        out_buffer = shiftLeft(out_buffer, stride); // outbuffer = 512
...
//            real[k] = mask[k] * (float) Math.cos(phase[k]);
...
//            imag[k] = mask[k] * (float) Math.sin(phase[k]);
        }
    ...
    private double[] audioToBuffer(double[] buffer, double[] audio) {
        System.arraycopy(audio, 0, buffer, 0, audio.length);
